...
30 9 * * * source ../jwells/Sites/bin/activate ; cd ../../../Applications/MAMP/htdocs/pla-feed ; python pla-feed.py -d vcf
30 9 * * * source ../jwells/Sites/bin/activate ; cd ../../../Applications/MAMP/htdocs/pla-feed ; python pla-feed.py -d asf
...
def fileUpload():	 	
	...
	with pysftp.Connection(host='home249957217.1and1-data.host', username='u49834858', password='asi#4300', port=22, cnopts=cnopts) as sftp:
	...
	print "Bing FTP Upload Started: " + str(filenameCSV) + " Filesize: " + str(fileSizeCSV)	
	session = ftplib.FTP('feeds.adcenter.microsoft.com','resolution','TS4icxZ5Ln8TU*e')
	#print "FTP Upload Started: " + str(filenameXML) + " Filesize: " + str(fileSizeXML)
	file = open(fullpathCSV,'rb')                  # file to send
	session.storbinary('STOR '+ filenameCSV, file)     # send the file
	file.close()                                    # close file and FTP
	session.quit()
	# print the successful upload message
...
if r.status_code == 200:
	...
	#fileUpload()
	#print "File Upload Finished: \n" + filenameXML + " Filesize: " + str(fileSizeXML) + "\n"	
